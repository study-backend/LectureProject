...
=======
>>>>>>> 1196a3b [Ï†ï Ï§ÄÏÉÅ] 1. DAOÎ•º ÏÑúÎπÑÏä§ Ìò∏Ï∂ú ÌõÑ Ïª®Ìä∏Î°§ÏóêÏÑú Ìò∏Ï∂ú ÏûëÏóÖ ÏôÑÎ£å
...
//////////////////////insert
	public void insert(List<Teacher> list) {
		...
		try {
			con = DbUtil.getConnection();
			con.setAutoCommit(false); // ¿⁄µø commit ≤˚
			...
			for (Teacher r : list) {
				...
				ps.setDate(7, sqlDate); // ª˝º∫ Ω√∞£
				ps.setDate(8, sqlDate); // ºˆ¡§ Ω√∞£

				ps.addBatch(); // OraclePreparedStatementø° batch∑Œ øœº∫µ» SQL √ﬂ∞°?
				ps.clearParameters(); // OraclePreparedStatementø° ¡ˆ¡§µ» Parameter∞™ √ ±‚»≠?
			}

			ps.executeBatch(); // ¥©¿˚µ» batch Ω««‡
			ps.clearBatch(); // ¥©¿˚µ» batch √ ±‚»≠
			con.commit(); // Commit«œø© ¿˚øÎ
...
//////////////////////update
	public void update(Teacher teacher) {
		...
		try {
			con = DbUtil.getConnection(); // ø¨∞…
			ps = con.prepareStatement(sql); // SQLπÆ ¡ÿ∫Ò
